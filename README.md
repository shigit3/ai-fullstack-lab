...
```text
使用 $ai-fullstack-learning，开始学习
```

如果你已经明确想学某个单元，也可以直接指定：
...
- 这是独立课程。你可以把它安排在任何固定学习时间，时间安排不是课程依赖关系。
- 默认一个对话聚焦一个单元；单元没学完就在当前对话里接着提问和写代码，不需要额外说固定口令。
- 只维护一个当前学习状态，避免在多个学习方向之间来回切换。
- 每个单元都有可交付物、checkpoint、验证方式和完成边界。
- 不按天数推进，只按 unit 完成情况推进。
- 默认由 Codex 创建 starter、记录进度、判断完成并收尾，不把协调负担丢给学习者。
...
只有在你换了新对话、但上一个单元确实还没完成时，才需要说“恢复当前单元”。这是跨对话恢复用的，不是日常学习时必须说的命令。
